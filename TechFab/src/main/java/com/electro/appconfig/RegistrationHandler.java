...
public class RegistrationHandler {
	...
	public String validateDetails(UserDetails userDetails,MessageContext messageContext){
		...
		int countUser = userDetailsService.checkUserName(userDetails.getUserName());
		int countEmail = userDetailsService.checkEmail(userDetails.getEmail());
		...
		if(countEmail == 1)
		{
			messageContext.addMessage(new MessageBuilder().error().source("email").defaultText("Email Id is already Used").build());
			status="failure";
		}
